...
$(() => {
  ...
  $("#table").serenityTable({
    dataSource: new serenity.DataSource({ 
      data: employees,
      change: function (args : serenity.DataSourceChangeEventParams) {
        // Concat the sort objects together for before and after the sort occurred.
        let before: string = Enumerable.From(args.before).Where("$.dir.length > 0").Select("$.field + ' sorted ' + $.dir").ToString(" and ");
        let after: string = Enumerable.From(args.value).Where("$.dir.length > 0").Select("$.field + ' sorted ' + $.dir").ToString(" and ");
